...
def create_user(db: Session, user: schemas.UserCreate):
  ...
  db.add(new_user)
  db.commit()
  return schemas.UserBase(id=new_user.id, username=new_user.username, email=new_user.email)

#articles
def get_articles(db: Session):
  articles = select(models.Article.id, models.Article.title, models.Article.content, models.User.username).join(models.User)
...
def edit_article(db: Session, content: str, user_id: int, article_id: int):
  ...
  db_article = db.execute(stmt).one()
  if db_article is None:
    return False
  else:
    stmt = update(models.Article).where(models.Article.id == article_id).values(content=content)
    db.execute(stmt)
    db.commit()
    updated_article = schemas.ArticleBase(title=db_article.title, content=content, username=db_article.username)
    return updated_article

def edit_article_title(db: Session, title: str, user_id: int, article_id: int):
  stmt = select(models.Article.title, models.Article.content, models.User.username
  ).join(models.User
  ).where(models.Article.id == article_id
  ).where(models.Article.user_id == user_id)
  ...
  if db_article is None:
    ...
    stmt = update(models.Article).where(models.Article.id == article_id).values(title=title)
    db.execute(stmt)
    db.commit()
    updated_article = schemas.ArticleBase(title=title, content=db_article.content, username=db_article.username)
    return updated_article
